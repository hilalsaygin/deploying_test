...
module.exports = {
  ...
  register: async (req, res) => {
    ...
    if (!exists) {
      if (!validator.isEmail(req.body.email)) {
        res
          .status(400)
          .json({ msg: "Please provide a valid email", data: null });
        return;
      }
      ...
      const data = new UserModel({
        name: req.body.name,
        nickname: req.body.nickname || req.body.email,
  ...
  logout: (req, res) => {
    if (req.session) {
      ...
      // destroy the session data in the MongoDB store
      mongoDBstore.destroy(req.session.id, (err) => {
        if (err) {
          console.log("Error deleting session from MongoDB store:", err);
        } else {
          console.log("Session data deleted from MongoDB store");
        }
      });
      ...
      res.status(400).json({ success: false, msg: "req.session is null" });
    }
  },
    ...
    // If "toDelete" is defined, delete influencer from favorites
      ...
      if (influencer) {
        ...
        const index = user.favorites.findIndex(
          (favorite) => favorite._id.toString() === influencer._id.toString()
        );
        if (index !== -1) {
  ...
  resetPassword: async (req, res) => {
    try {
      ...
      const link =
        "https://leetcode.com/accounts/password/reset/key/3xnty-67d-453a65ca08e0c71d538f/";
  ...
  updateName: async (req, res) => {
    ...
    res
      .status(200)
      .json({ success: true, msg: "Successfully updated.", data: user });
  },
